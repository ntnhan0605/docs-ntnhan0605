function fakeGetData(url, callbackFn) {
	var fakeResponses = {
		"https://zendvn.com/api/1": "Data1",
		"https://zendvn.com/api/2": "Data2",
		"https://zendvn.com/api/3": "Data3"
	};
	var randomMiliseconds = Math.floor(Math.random() * 5000) + 1000;

	console.log(`Đang gọi API với URL : ${url} - Thời gian ${randomMiliseconds}ms`);

	setTimeout(function(){
		callbackFn(fakeResponses[url]);
	}, randomMiliseconds);
}

/**
 * Yêu cầu Challenge 02:
 *  1. Các lời gọi API sẽ có thời gian chạy khác nhau và ngẫu nhiên.
 *  2. Cả 3 lời gọi API phải được chạy đồng thời cùng một lúc.
 *  3. Chỉ áp dụng các kiến thức cơ bản của Javascript (Không dùng Promise)
 *  4. API nào chạy xong phải in kết quả ra ngay (Nhưng phải đảm bảo đúng thứ tự 1->2->3)
 */

// API1  API2  API3
//  1s    2s    3s
//  1s    3s    2s
//  2s    1s    3s
//  2s    3s    1s
//  3s    1s    2s

// ??? Pattern

function startRunAPI(url) {
  let res = null;
  let callback = null;

  fakeGetData(url, function(data) {
    if (callback !== null) {
      callback(data); // cbFn(res)
    } else {
      res = data;
    }
  })

  return function getData(cbFn) {
    if (res !== null) {
      cbFn(res);
    } else {
      callback = cbFn;
    }
  }
}

let fnGetData1 = startRunAPI('https://zendvn.com/api/1'); // thunk
let fnGetData2 = startRunAPI('https://zendvn.com/api/2'); // thunk
let fnGetData3 = startRunAPI('https://zendvn.com/api/3'); // thunk

fnGetData1(function(res1) {
  console.log(res1)
  fnGetData2(function(res2){
    console.log(res2)
    fnGetData3(function(res3){
      console.log(res3)
    })
  })
})