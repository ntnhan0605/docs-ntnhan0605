    ...
    <div class="relative" style="width: 320px; max-height: 475px; background: white">
      ...
      <div v-if="!isShowThankYou" class="relative" style="max-height: 475px; overflow: auto">
        <div style="min-height: 118px">
          <ImageBlock :placeholder-size="imageSize.DesktopLightBoxA" :block="imageBlocks.imageMobile" ref="editblock-static-image-desktop"></ImageBlock>
        </div>
          ...
          <div class>
            <draggable v-model="listBlockAdded">
              <div v-for="(item, indexBlock) in listBlockAdded" :key="'edit' + item.name + '-' + item.id">
                <component class="padding-x-sm margin-bottom-sm" :ref="'editblock-' + item.name + '-' + item.id" @remove="onRemoveBlock" :block="item" :index="indexBlock" :is="item.name" v-show="item.name === 'LinkButtonBlock'"></component>
              </div>
            </draggable>
