<template>
  <div>
    <div @click="emitSelect" class=" cursor-grab le-hover-border" :class="{'mark-error-value': !block.data.value, 'newbuilder-selected': currentSelectEdit == portalID }">
      <div class="font-semibold mb-2 mt-2">{{ block.data.placeholder }}</div>
      <div>
        <div v-for="(i, index) in block.data.listQuestion" class="flex items-center margin-bottom-xs" :key="portalID + '-question-' + index">
          <div class="mr-2" style="border: 1px solid #3598EC; width: 20px; height: 20px; border-radius: 50%;flex: 0 0 20px;"></div>
          {{ i.placeholder }}
        </div>
      </div>
    </div>
