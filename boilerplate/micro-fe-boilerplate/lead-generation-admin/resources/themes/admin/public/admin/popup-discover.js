...
var LEAD_VERSION = '2.1-aka-3.0.63  - 🌠 The Starry Night';
var LEAD_IS_DEV = false;
var LEAD_DEV_URL = '';
var LEAD_SCROLL_EVENT;
...
if (LEAD_API.includes('localhost')) {
  ...
  LEAD_DEV_URL = 'http://localhost:8080/';
}

console.log('===ryan===> LEAD_SITE_DOMAIN : ', LEAD_SITE_DOMAIN);

if (LEAD_SITE_DOMAIN.includes('marrybaby.vn')) {
  LEAD_CLASS = 'pink';
  console.log('===ryan===> LEAD_CLASS : ', LEAD_CLASS);
}
...
var LEAD_CURRENT_URL = 'https://' + location.hostname + location.pathname;
...
function LEAD_RESET_VAR() {
  ...
  CAMPAIGN_SHOW = {};
  CAMPAIGN_UUID = '';
  CAMPAIGN_IMPRESSION_ACTION = '';
  // LEAD_CURRENT_URL = location.href.toLowerCase();
  LEAD_CURRENT_URL = 'https://' + location.hostname + location.pathname;
  // For config axios
  //  LEAD_AXIOS_CONFIG = {};

  LEAD_POPUP_START_TIME = null;
  LEAD_POPUP_END_TIME = null;
}

function LOAD_LEAD_SCRIPT() {
  console.info('🚀 LOAD_LEAD_SCRIPT 🤟 ');
  ...
  script.onload = function () {
    ...
    jQuery
      ...
      .done(function () {
        ...
        // TODO add function check scroll
        if (window.next) {
          const debounced = _.debounce((e) => {
            console.log('--🌻 routeChangeComplete', e, Date.now());
            const currentPath = location.pathname;
            // Remove floating, inline when url change
            jQuery('.lead-modal-wrapper').remove();
            if (jQuery(`[data-popup-url="${currentPath}"]`).length) {
              console.log('🔥 This URL page has show popup!');
            } else {
              console.log('--Process checkShowPopup');
              checkShowPopup();
            }
            if (LEAD_SCROLL_EVENT) {
              jQuery(window).off('scroll', LEAD_SCROLL_EVENT);
            }
          }, 500);

          next.router.events.on('routeChangeComplete', debounced);
        }
          ...
          axios
            ...
            .then((data) => {
              ...
              if (data._status == 1) {
                ...
                console.log('--listCampaignValid : ', listCampaignValid);
                if (listCampaignValid && listCampaignValid.length) {
                  ...
                  if (Number.isInteger(scrollRule)) {
                    ...
                    const $p = (jQuery(`[data-url="${location.pathname}"]`).length && jQuery(`[data-url="${location.pathname}"]`)) || jQuery('body');
                    ...
                    LEAD_SCROLL_EVENT = function leadScollCheckHandler() {
                      ...
                      console.log('---Scroll', percent, scrollRule);
                    ...
                    jQuery(window).scroll(LEAD_SCROLL_EVENT); //end scroll event
                  }
                } else {
    ...
    function leadShowPopup(campaignShow) {
      if (!campaignShow.extra_fields) {
        return;
      }
      // newbuilder
      LEAD_POPUP_START_TIME = Date.now();
      if (campaignShow.extra_fields && campaignShow.extra_fields.popupVersion == '2.1') {
        ...
        <div class="lead-modal__wrapper animate__animated ">
          <div class="lead-modal__content">
            ...
            <div class="lead-modal__body">
              <h2 class="lead-modal__title ${LEAD_CLASS}">${popUpTitle}</h2>
              <p class="lead-modal__description ${LEAD_CLASS}">${popUpDesc}</p>
              <form class="lead-modal__form">
                ...
                <div class="lead-modal__checkbox-wrapper ${LEAD_CLASS}">
                  ...
                  <label for="checkbox" class="lead-modal__checkbox__label">
                    ${popUpTermAgreeText} <span style="font-weight: bold;"><a target="blank" href="${popUpTermURL || 'https://hellobacsi.com/chinh-sach'}" style="text-decoration: none;" rel="noopener noreferrer">${popUpTermText}</a></span>
                  </label>
                </div>
          ...
          <button class="lead-modal__close-btn js-modal__close ${LEAD_CLASS}">
  ...
  `);
    ...
    function leadShowPopupNewbuilder(campaignShow) {
      ...
      try {
        ...
        const popupID = 'lead-modal-' + uuidv1();
        ...
        // TODO: list category template
        function getCategoryLevel(t = 'Xin chao {{category_2}} extra') {
          if (t.includes('{{category_')) {
            const cl = '{{category_'.length;
            const sc = t.search('{{category_');
            const sf = t.charAt(cl + sc);
            return +sf;
          } else {
            return null;
          }
        ...
        let titleText = _.get(textBlocks, 'titleBlock.data.value', '');
        categoryLevel = getCategoryLevel(titleText);
        if (categoryLevel) {
          console.log('⚡️ Has categoryLevel', categoryLevel);
          const listMeta = Array.from(document.getElementsByTagName('META'));
          const valueMeta = listMeta.find((i) => i.name === `content_category_${categoryLevel}`);
          const valueMetaEn = listMeta.find((i) => i.name === `content_category_en_${categoryLevel}`);
          if (valueMeta && valueMetaEn) {
            categoryData[categoryLevel] = {
              value: valueMeta.content,
              value_en: valueMetaEn.content,
            };
            titleText = titleText.replace(`{{category_${categoryLevel}}}`, valueMeta.content);
            console.log('⚡️ Set categoryData ', categoryLevel);
          } else {
            console.log('⚡️ No META TAG for categoryLevel, not show popup', categoryLevel);
            return;
          }
        } else {
          console.log('⚡️ No categoryLevel', categoryLevel);
        }
        listBlockAdded.forEach((b) => {
          if (b.name == 'TitleBlock') {
            listFieldAddedHTML += `<h2 class="lead-modal__title ${'le-text-align-' + _.get(b.data, 'align')} ${LEAD_CLASS}">${b.data.value}</h2>`;
          }
          if (b.name == 'SubtitleBlock') {
            listFieldAddedHTML += `
        <p class="lead-modal__description ${'le-text-align-' + _.get(b.data, 'align')} ${LEAD_CLASS}">${b.data.value}</p>
        `;
          }
          if (b.name == 'TextBlock') {
            listFieldAddedHTML += `
        ...
        <div>
          <a target="${b.data.newtab ? 'blank' : 'parrent'}" href="${b.data.url}" style="text-decoration: none;">
            <button type="button" class="lead-modal__btn js-modal__close js-popup-other-submit ${LEAD_CLASS}"> ${b.data.value} </button>
          </a>
        </div>
        `;
          }
      ...
      `;
          ...
          // TODO add modal id
          popupHtml = `
      <div id="${popupID}" data-popup-url="${currentPath}" data-popup-type="lightbox-inline" class="lead-modal lead-modal-wrapper modal--animate-scale ${isInPage ? 'le-buider-inline' : ''} ${isSlider ? 'le-buider-slider' : ''}">
        ...
        <div class="lead-modal__wrapper le-newbuilder-wrap-${currentLayout}">
            ...
            <div class="lead-modal__body le-newbuilder-overlay">
              <form class="lead-modal__form ${popupID}">
                <div style="margin-bottom: 20px;">
                  <h2 class="lead-modal__title ${'le-text-align-' + _.get(textBlocks.titleBlock.data, 'align')} ${LEAD_CLASS}">${titleText}</h2>
                  <p class="lead-modal__description ${'le-text-align-' + _.get(textBlocks.titleBlock.data, 'align')} ${LEAD_CLASS}">${textBlocks.subtitleBlock.data.value}</p>
                </div>
          ...
          ${
            isShowClose
              ? `
                <button class="lead-modal__close-btn ${currentLayout == 'Fullscreen' ? 'lead-fullscreen-desktop-close-btn' : ''} js-modal__close ${LEAD_CLASS}">
                  <svg class="lead-modal__close-icon" viewBox="0 0 24 24">
                    <g fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="2"><line x1="3" y1="3" x2="21" y2="21" /><line x1="21" y1="3" x2="3" y2="21" /></g>
                  </svg>
                </button>`
      ...
      `;
          function renderHTML($child, popupHtml, tag, pos) {
            const $pos = $child.filter(tag).eq(pos);
            if ($pos.length) {
              jQuery(popupHtml).insertAfter($pos);
            } else {
              jQuery(popupHtml).insertAfter($child.eq($child.filter(tag).length - 1));
            }
          }
          // TODO: Show popup
          ...
          if (currentLayout.includes('Inline')) {
            const $content = jQuery(
              `[data-url="${location.pathname}"] .body-content`,
              // '.entry-content-body,.article-details-content-main.single-article-details,.page-details-content-main.article-main-content,.body-content,.blog-details-content-main'
            );
            if ($content.length) {
          ...
          if (currentLayout == 'Sidebar') {
            const $sideBar = jQuery('.hc2-sidebar-with-ads,.page-main-col .right-col.f-right,.hhg-lead-sidebar');
            if ($sideBar.length) {
          ...
          jQuery('.lead-modal__form.' + popupID).validate({
            ...
            submitHandler: function (form) {
              submitHandler(form, categoryLevel, categoryData, popupID);
            },
          });
        } // End LightBox
        ...
        <aside class="le-only-desktop">
          <div class="le-skin-a-desktop">
            <div style="position: relative;">
              ...
              <a style="position: absolute; top: 75%; left: 50%; transform: translate(-50%, -50%); text-align: center; width: calc(100% - 15px);" href="${actionBlocks.linkButtonBlock.data.url}" target="${actionBlocks.linkButtonBlock.data.newtab ? 'blank' : '_parent'}">
                <button class="le-joinbow-btn js-popup-other-submit">${actionBlocks.linkButtonBlock.data.value}</button>
              </a>
              <button class="lead-modal__close-btn js-modal__close ${LEAD_CLASS}"><svg class="lead-modal__close-icon" viewBox="0 0 24 24"> <g fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="2"><line x1="3" y1="3" x2="21" y2="21" /><line x1="21" y1="3" x2="3" y2="21" /></g> </svg></button>
            </div>
          </div>
        </aside>
        `;
        ...
        `;
          ...
          jQuery('body').prepend(`
            <div id="${popupID}" class="lead-modal-wrapper" data-popup-url="${currentPath}" data-popup-type="skin_ab">
              ${desktopHTMLPopup}
              <aside class="le-only-mobile">
                <div class="le-skin-mobile">
                  <div style="position: relative;">
                    <img src="${campaignShow.image}" style="width: 100%;">
                    <a style="position: absolute; top: 82%; left: 50%; transform: translate(-50%, -50%); text-align: center; max-width: calc(100% - 15px);" href="${actionBlocks.linkButtonBlock.data.url}" target="${actionBlocks.linkButtonBlock.data.newtab ? 'blank' : '_parent'}">
                      <button class="le-joinbow-btn-mobile js-popup-other-submit">${actionBlocks.linkButtonBlock.data.value}</button>
                    </a>
                    <button class="le-newbuilder-close-button le-abs-top-right js-modal__close">
        ...
        if (currentLayout == 'Floating') {
          ...
          jQuery('body').append(`
      <div id="${popupID}" class="lead-modal-wrapper lead-floating-wrapper" data-popup-url="${currentPath}" data-popup-type="floating">
       <aside class="le-only-desktop">
        <div class="le-floating-desktop">
          <div class="le-floating-desktop-wrap" style="position: relative; background: #fff;">
            ${imgFloatDesktop}
            <div style="padding:20px;">
              <div class="le-floating-desktop-title le-text-align-${textBlocks.titleBlock.data.align}">${titleText}</div>
            ...
            <a target="${actionBlocks.linkButtonBlock.data.newtab ? 'blank' : '_parent'}" style="margin-left: 35px;text-decoration: none;" href="${actionBlocks.linkButtonBlock.data.url}">
       ...
       <div class="le-floating-mobile">
         <div class="le-floating-mobile-wrap" style="position: relative; background: #fff;">
           <aside style="display:flex;justify-content: space-between;">
            <div>
              <div class="le-floating-mobile-title le-text-align-${textBlocks.titleBlock.data.align}">${titleText}</div>
           ...
           <div style="text-align:center; margin-top: 10px;">
            <a target="${actionBlocks.linkButtonBlock.data.newtab ? 'blank' : '_parent'}"  href="${actionBlocks.linkButtonBlock.data.url}" style="text-decoration: none;"><button class="le-joinbow-floating-btn js-popup-other-submit">${actionBlocks.linkButtonBlock.data.value}</button></a>
           </div>
      ...
      `);
        ...
        if (currentLayout == 'Tab') {
          jQuery('body').append(`
      <div id="${popupID}" class="lead-modal-wrapper" data-popup-url="${currentPath}" data-popup-type="tab">
      ...
      `);
        ...
        if (currentLayout == 'CenterTab') {
          jQuery('body').append(`
      <div id="${popupID}" class="lead-modal-wrapper" data-popup-url="${currentPath}" data-popup-type="center_tab">
      ...
      `);
        ...
        jQuery('body').on('click', '.js-popup-other-submit', function () {
          ...
          if (categoryLevel) {
            data['category'] = categoryData[categoryLevel].value_en;
          }
    ...
    // --EVENTS LEAD POPUP
    function submitHandler(form, categoryLevel, categoryData, popupID) {
      console.log('submitHandler', categoryLevel, categoryData);
      ...
      let data = {
        ...
        title_article: document.title,
        meta: formData.map((i) => ({
          key: jQuery(`.lead-modal__form.${popupID} [name="${i.name}"]`).data('key'),
          ...
          control: i.name.replace('1', '').replace('2', '').replace('3', ''),
          is_sent_mailchimp: jQuery(`.lead-modal__form.${popupID} .mailchimp-${jQuery(`.lead-modal__form.${popupID} [name="${i.name}"]`).data('key')}`).data('mailchimp'),
        })),
        cookie_id: CAMPAIGN_UUID,
        ga_client_id: gaCookie,
      };
      ...
      // TODO: categoryLevel
      if (categoryLevel) {
        if (categoryData[categoryLevel].value_en) {
          data['category'] = categoryData[categoryLevel].value_en;
        }
      }
      ...
      if (CAMPAIGN_SHOW.extra_fields.popupVersion == '2.1') {
        ...
        linkHTML = linkButtonBlock ? ` <div><a href="${linkUrl}" target="${linkNewtab ? '_blank' : 'parrent'}"><button class="lead-modal__btn ${LEAD_CLASS}">${linkText}</button></a></div> ` : '';
      ...
      if (isShowThanksYou) {
        const contentEl = `#${popupID} .lead-modal__content`;
        jQuery(contentEl).empty();
        jQuery(contentEl).append(`
    ...
    jQuery('body').on('change', '#lead-term-checkbox', function (e) {
    ...
    function closePopupHandler(e) {
      ...
      jQuery('#lead-modal').remove();
      if (jQuery(this).parent().parent().length) {
